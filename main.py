...
def run_pipeline(pdf_path: str, job_id: str, mode: str, language: str):
    ...
    try:
        if mode == "review":
            ...
            ok = pipeline_video.run_auto(pdf_path, output_video, output_folder, language=language)
...
def run_render(job_id: str):
    ...
    try:
        ...
        ok = pipeline_video.phase_render(output_folder, output_video, language=cfg.get("language", "en"))
        if not ok or not os.path.exists(output_video):
...
async def root():
    return {
        ...
        "status": "running",
        "endpoints": {
            "upload_ai": "POST /upload (PDF, form: mode=auto|review, language)",
...
findstr "upload-manual" main.py
findstr "import fitz" main.py
