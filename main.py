...
def _write_config(output_folder, language, cartesia_api_key, cartesia_voice_id):
...
def _read_config(output_folder):
    ...
    if not os.path.exists(config_path):
        return {"language": "en", "cartesia_api_key": None, "cartesia_voice_id": None}
...
def run_pipeline(pdf_path: str, job_id: str, mode: str, language: str, cartesia_api_key: str, cartesia_voice_id: str):
    ...
    try:
        if mode == "review":
            ...
            ok = pipeline_video.phase_script(pdf_path, output_folder, language=language)
            if not ok:
...
def run_render(job_id: str):
    ...
    try:
        ...
        pipeline_video.phase_render(
            ...
            cartesia_voice_id=cfg.get("cartesia_voice_id"),
...
async def upload_manual(
    ...
    status_file = f"{output_folder}/status.json"
    _update_status(status_file, "rendering", 60)
    log(f"Uploaded manual job: {job_id} ({len(slides)} slides, language={language})")
...
async def root():
    return {
        ...
        "status": "running",
        "endpoints": {
            "upload_ai": "POST /upload (PDF, mode=auto|review, language, cartesia_api_key?, cartesia_voice_id?)",
            "upload_manual": "POST /upload-manual (slides[], scripts=JSON array, language, cartesia_api_key?, cartesia_voice_id?)",
