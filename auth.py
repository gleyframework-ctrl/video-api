...
async def signup(user: UserSignup):
    try:
        ...
        return {"message": "Account created", "user_id": auth_response.user.id, "email": auth_response.user.email}
...
async def get_user_settings(user_id: str):
    try:
        response = supabase.table("user_profiles").select("cartesia_api_key, cartesia_voice_id").eq("id", user_id).execute()
...
@router.delete("/user/videos/{job_id}")
async def delete_video_db(job_id: str, user_id: str):
    try:
        supabase.table("videos").delete().eq("job_id", job_id).eq("user_id", user_id).execute()
        return {"message": "Video record deleted"}
    except Exception as e:
...
async def change_password(data: dict):
    ...
    if not user_id or not new_password or len(new_password) < 6:
        raise HTTPException(status_code=400, detail="Invalid password requirements")
